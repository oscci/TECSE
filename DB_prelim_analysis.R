...
myfile<-paste(myname,".xlsx",sep="") #convert their ID into an excel filename - 'paste' joins things together so sticks on the filetype
...
plot(aggdatam$Ageband,aggdatam$TRC.SI,ylim=c(.4,1),main='Subject Intransitive')
lines(aggdatam$Ageband,aggdatam$TRC.SI,col='blue',type='o',pch=18)
lines(aggdatam$Ageband,aggdatam$TEC.SI,col='red',type='o',pch=19)

plot(aggdatam$Ageband,aggdatam$TRC.ST,ylim=c(.4,1),main='Subject Transitive')
lines(aggdatam$Ageband,aggdatam$TRC.ST,col='blue',type='o',pch=18)
lines(aggdatam$Ageband,aggdatam$TEC.ST,col='red',type='o',pch=19)

plot(aggdatam$Ageband,aggdatam$TRC.O,ylim=c(.4,1),main='Object')
lines(aggdatam$Ageband,aggdatam$TRC.O,col='blue',type='o',pch=18)
lines(aggdatam$Ageband,aggdatam$TEC.O,col='red',type='o',pch=19)

plot(aggdatam$Ageband,aggdatam$TRC.Obl,ylim=c(.4,1),main='Oblique')
lines(aggdatam$Ageband,aggdatam$TRC.Obl,col='blue',type='o',pch=18)
lines(aggdatam$Ageband,aggdatam$TEC.Obl,col='red',type='o',pch=19)

plot(aggdatam$Ageband,aggdatam$TRC.IO,ylim=c(.4,1),main='Indirect Object')
lines(aggdatam$Ageband,aggdatam$TRC.IO,col='blue',type='o',pch=18)
lines(aggdatam$Ageband,aggdatam$TEC.IO,col='red',type='o',pch=19)



#make pass fail scores; convert scores of 4-choice so 3->1, 4->2; convert scores of 2choice so 8->1 and 9-10 ->2
myrawdata[,106:115]<-0
colnames(myrawdata)[106:115]<-c('TRC.SI.pf','TRC.ST.pf','TRC.O.pf','TRC.Obl.pf','TRC.IO.pf',
                                'TEC.SI.pf','TEC.ST.pf','TEC.O.pf','TEC.Obl.pf','TEC.IO.pf' )
for (j in 95:99){
...
jpeg("plots3.jpg")
par(mfrow=c(2,1))
temp<-table(myrawdata$ageband,myrawdata$TRC.O.pf)
TRC_O_tab<-temp/rowSums(temp)
barplot(t(TRC_O_tab), main="TRC_Object",
        xlab="Age band", col=c("pink","red","purple")) 
temp<-table(myrawdata$ageband,myrawdata$TEC.O.pf)
